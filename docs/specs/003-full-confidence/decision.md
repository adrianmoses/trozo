...
| Criterion 1: seed-verified labels never change                                                    | Holds in code and unit tests; not observable live until bug 001 was fixed                                                         | The seed index was empty outside Docker (path bug, see Context)                                                                             |

---

## Spec Gaps Exposed <!-- optional -->

- **Seed path bug (since 001): fixed in [bugs/001](../../../bugs/001-seed-path.md).** `DEFAULT_SEED_PATH` now climbs `parents[4]`, and cached fast and full responses recompute the seed signal on read, so stale `seed: false` entries are corrected without eviction. After the fix, fast mode verifies 13/20 dev and 14/24 test chunks (was 0/20); 003's seed-masked metrics are unchanged.
